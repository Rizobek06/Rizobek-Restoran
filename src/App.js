...
import Dishes from "./components/dishes-list/Dishes";
...
function App() {
  ...
  const getFilterText = (fitertext) => {
    if (fitertext.toLowerCase() === "barcha mahsulotlar") {
      setData(dishes);
      return;
    }

    const filterData = dishes.filter(
      (item) => item.category.toLowerCase() === fitertext.toLowerCase()
    );
